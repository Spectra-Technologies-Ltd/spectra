...
import {
  MapPin, Search, Plus, Filter, MoreVertical, AlertTriangle, Users,
  X, Edit, Trash2,
} from 'lucide-react';
        ...
        {isLoading ? (
          ...
          <>
                ...
                <tbody className="divide-y divide-border">
                  {data?.data?.map((site: Site) => (
                    <tr key={site.id} className="hover:bg-secondary/30 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <Badge colorClassName={getRiskLevelColor(site.riskLevel)}>{site.riskLevel}</Badge>
                      </td>
                      <td className="px-6 py-4">
            ...
            <div className="md:hidden divide-y divide-border">
              {data?.data?.map((site: Site) => (
                <div key={site.id} className="p-4 flex items-start gap-3 active:bg-secondary/30">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-medium text-foreground truncate">{site.name}</span>
                      <Badge colorClassName={getRiskLevelColor(site.riskLevel)}>{site.riskLevel}</Badge>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1 truncate">{site.address}</p>
                    <p className="text-xs text-muted-foreground mt-1">{site.client?.companyName}</p>
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className={cn("text-xs", site._count.incidents > 0 ? "text-rose-400 font-medium" : "text-muted-foreground")}>{site._count.incidents} incidents</span>
                      </div>
                    </div>
                  </div>
